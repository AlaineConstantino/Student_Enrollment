    ...
    <div class="form-container">
        <div class="form-card">
            <div class="form-header">
                <h2><i class="bi bi-plus-circle me-2"></i>Record New Payment</h2>
            </div>
            ...
            <form action="{{ route('payments.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('student_id') is-invalid @enderror" id="student_id" name="student_id" required>
                        ...
                        @foreach($students as $student)
                            <option value="{{ $student->student_id }}" @if(old('student_id') == $student->student_id) selected @endif>
                                {{ $student->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('grade_id') is-invalid @enderror" id="grade_id" name="grade_id" required>
                        ...
                        @foreach($grades as $grade)
                            <option value="{{ $grade->grade_id }}" @if(old('grade_id') == $grade->grade_id) selected @endif>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('payment_amount') is-invalid @enderror" id="payment_amount" name="payment_amount" value="{{ old('payment_amount') }}" required>
                    @error('payment_amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-submit btn-lg"><i class="bi bi-check-circle me-2"></i>Record Payment</button>
