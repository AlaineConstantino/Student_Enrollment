...
<head>
    ...
    <title>Create Grade - Admin</title>
    ...
    <div class="form-container">
        <div class="form-card">
            <div class="form-header">
                <h2><i class="bi bi-plus-circle me-2"></i>Create New Grade</h2>
            </div>
            ...
            <form action="{{ route('grades.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('grade_number') is-invalid @enderror" id="grade_number" name="grade_number" value="{{ old('grade_number') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('section_id') is-invalid @enderror" id="section_id" name="section_id" required>
                        ...
                        @foreach($sections as $section)
                            <option value="{{ $section->class_id }}" @if(old('section_id') == $section->class_id) selected @endif>
                                {{ $section->class_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('subject_id') is-invalid @enderror" id="subject_id" name="subject_id" value="{{ old('subject_id') }}">
                    @error('subject_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label for="fee_amount" class="form-label">Fee Amount *</label>
                    <input type="number" step="0.01" class="form-control @error('fee_amount') is-invalid @enderror" id="fee_amount" name="fee_amount" value="{{ old('fee_amount') }}" required>
                    @error('fee_amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-submit btn-lg"><i class="bi bi-check-circle me-2"></i>Create Grade</button>
