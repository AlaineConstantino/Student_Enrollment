...
<body>
    <div class="container-fluid py-4">
        <div class="management-header">
            <h2 style="color: var(--phthalo-green); font-weight: bold;">Teachers Management</h2>
            <a href="{{ route('teachers.create') }}" class="btn btn-lg" style="background-color: var(--phthalo-green); color: white;">
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                <form method="GET" action="{{ route('teachers.index') }}" class="search-box">
        ...
        <div class="tab-content">
            <div class="tab-pane fade @if(!request('archived')) show active @endif" id="active" role="tabpanel">
                @if($teachers->count())
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Phone</th><th>Qualification</th><th>Joined</th><th>Actions</th></tr></thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('teachers.edit', $teacher) }}" class="btn btn-sm btn-outline-primary me-2"><i class="bi bi-pencil"></i> Edit</a>
                                        <form action="{{ route('teachers.destroy', $teacher) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Archive?');"><csrf><method DELETE><button type="submit" class="btn btn-sm btn-outline-warning"><i class="bi bi-archive"></i> Archive</button></form>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    <div class="empty-state"><i class="bi bi-inbox" style="font-size: 3rem; margin-bottom: 1rem; display: block; color: #d1d5db;"></i><p>No active teachers found.</p></div>
                @endif
            </div>
            ...
            <div class="tab-pane fade @if(request('archived')) show active @endif" id="archived" role="tabpanel">
                @if($archived && $archived->count())
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Phone</th><th>Archived</th><th>Actions</th></tr></thead>
                            <tbody>
                                @foreach($archived as $teacher)
                                <tr style="opacity: 0.7;">
                                    <td>{{ $teacher->id }}</td>
                                    <td>{{ $teacher->name ?? 'N/A' }}</td>
                                    <td>{{ $teacher->email ?? 'N/A' }}</td>
                                    <td>{{ $teacher->phone ?? 'N/A' }}</td>
                                    <td>{{ $teacher->deleted_at->format('M d, Y') }}</td>
                                    <td>
                                        <form action="{{ route('teachers.restore', $teacher->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Restore?');"><csrf><button type="submit" class="btn btn-sm btn-outline-success"><i class="bi bi-arrow-counterclockwise"></i> Restore</button></form>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    <div class="empty-state"><i class="bi bi-inbox" style="font-size: 3rem; margin-bottom: 1rem; display: block; color: #d1d5db;"></i><p>No archived teachers.</p></div>
                @endif
            </div>
        </div>
    </div>
